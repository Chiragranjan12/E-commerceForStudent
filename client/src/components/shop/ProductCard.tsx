...
export function ProductCard({ id, title, price, image, category, isNew }: ProductCardProps) {
  return (
    <div className="group relative flex flex-col border-2 border-black bg-white transition-transform hover:-translate-y-1 hover:brutal-shadow duration-200">
      {/* Badge */}
      {isNew && (
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        <div className="flex justify-between items-start">
          <div>
            <p className="font-mono text-xs text-muted-foreground uppercase mb-1">{category}</p>
            <h3 className="font-display text-2xl font-bold leading-none tracking-tight uppercase">{title}</h3>
          </div>
          <span className="font-mono text-lg font-bold">{price}</span>
        </div>
      </div>
